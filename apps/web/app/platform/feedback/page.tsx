...
export default async function PlatformFeedbackPage({
  ...
  const releaseHistoryTopActorLabel = `Top planning actor: ${releaseHistoryTopActorTitle}, ${releaseHistoryTopActorEventLabel}`;
        ...
        <div className="grid gap-4">
          ...
          <div className="rounded-lg border border-neutral-800 bg-neutral-900">
            ...
            <div className="grid grid-cols-2 gap-2 border-b border-neutral-800 p-4 text-xs md:grid-cols-5">
              ...
              <div aria-label={releaseHistoryTopActorLabel} className="min-w-20 rounded-lg border border-neutral-800 bg-neutral-950 px-3 py-2">
              ...
              {inbox.rows.length > 0 ? (
                inbox.rows.map((row) => (
                  <div key={row.id} className="grid gap-2 px-4 py-3 text-sm md:grid-cols-[120px_minmax(0,1fr)_120px_90px_110px_220px] md:items-center">
                    ...
                    <a
                      href={buildFeedbackHref({ ...filters, selected: row.sourceMessageId })}
...
function buildFeedbackHref(filters: { status?: string; type?: string; appVersion?: string; selected?: string }) {
