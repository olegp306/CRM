...
export const CRM_ENTITY_EXTRACTOR_DEFAULT_PROMPT = [
  ...
  "Reminder signals include: напомни, поставь напоминание, создай задачу, задача, перезвонить, позвонить, написать, зафоллоуапить, вернуться, проверить, узнать, спросить, назначить встречу, созвониться, не забыть, если не ответит, когда ответит, после встречи, через неделю, через пару дней, через два дня, завтра, в пятницу, на следующей неделе, на следующей неделе во вторник, вечером, remind me, follow up, call back, schedule, set a task, check in, ping, ask again, in a couple of days, next Tuesday evening, if they do not reply.",
  "For natural reminder dates, treat через пару дней and через два дня as two days from now. Treat на следующей неделе во вторник as Tuesday of the next calendar week. When no exact time is provided, use утром=10:00, в обед=13:00, вечером=17:00.",
